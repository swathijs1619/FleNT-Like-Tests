...
etr1c.set_attributes("10mbit", "10ms", qdisc)  # from `r1` to `r2`
...
exp = Experiment("cubic_vs_ledbat")

# Configure one flow from `h1` to `h3` and another from `h2` to `h4`.
flow1 = Flow(h1, h3, eth3.get_address(), 0, 20, 1)
flow2 = Flow(h3, h1, eth1.get_address(), 0, 20, 1)
flow3 = Flow(h2, h4, eth4.get_address(), 0, 20, 1)
flow4 = Flow(h4, h2, eth2.get_address(), 0, 20, 1)

# Use `flow1` as a TCP flow. TCP CUBIC is default in Linux.
exp.add_tcp_flow(flow1, "cubic", 'CS0,CS0')
exp.add_tcp_flow(flow2, "cubic", 'CS0, CS0')
exp.add_tcp_flow(flow1, "cubic", 'CS1,CS1')
exp.add_tcp_flow(flow2, "cubic", 'CS1, CS1')

# Use `flow2` as a UDP flow, and set the rate at which it would send packets.
exp.add_tcp_flow(flow3, "ledbat", 'CS0,CS0')
exp.add_tcp_flow(flow4, "ledbat", 'CS0, CS0')
exp.add_tcp_flow(flow3, "ledbat", 'CS1,CS1')
exp.add_tcp_flow(flow4, "ledbat", 'CS1, CS1')
